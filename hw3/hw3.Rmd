...
```{r}
labevents_icu_tble = labevents_tble %>%
...
Get `itemid`s: 
```{r}
dlabitems = read_csv('/home/odamadafaka/mimic-iv-data/hosp/d_labitems.csv.gz')
...
for (x in labitems){
  ...
  .$itemid
itemid = labevents_icu_tble %>%
  filter(itemid %in% items_list) %>%
  count(itemid) %>%
  slice_max(n,1) %>%
  select(1,1) %>%data.frame()%>%.$itemid
itemids[i] = itemid
i = i + 1
}
...
```{r}
labevents_itids_tble = labevents_icu_tble %>% 
  subset(itemid %in% itemids) %>%
  print()
```

```{r}
  labevents_itids_tble %>%
  semi_join(sol_tble, by = c('subject_id', 'hadm_id')) %>%
  left_join(select(sol_tble, subject_id, hadm_id, intime),
  ...
  right_join(sol_tble, by = c('subject_id', 'hadm_id')) %>%
  ungroup() %>%
