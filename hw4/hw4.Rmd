...
```{r}
y_train = train$de30
x_train = as.data.frame(cbind(as.data.frame(train$gender), train$anchor_age, train$marital_status, train$ethnicity,
            train$potassium, train$sodium,train$chloride, train$bicarbonate, train$hematocrit,
            train$wbc, train$glucose, train$magnesium, train$calcium, train$heart_rate,train$non_invasive_blood_pressure_mean,train$non_invasive_blood_pressure_systolic,
...
rf <- randomForest(
  de30 ~  gender+ anchor_age+ marital_status+ ethnicity+ potassium+ sodium+chloride+ bicarbonate+ hematocrit+ wbc+ glucose+ magnesium+ calcium+heart_rate+non_invasive_blood_pressure_mean+non_invasive_blood_pressure_systolic+
...
predict(rf, test, type = 'response')
```

3. Compare model prediction performance on the test set.
```{r}
predict(fit, test,  type = c("link", "response", "terms"),)
```